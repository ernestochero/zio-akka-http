...
object ApiSpec extends ZioRouteTest {
  ...
  val apiLayer = (
    ((InMemoryItemRepository.test ++ InMemoryEventSubscriber.test) >>> ApplicationService.live) ++
      loggingLayer ++
  ...
  private def allItems: ZIO[Has[ApplicationService], Throwable, List[Item]] =
    ApplicationService.getItems.mapError(_.asThrowable)

  private val specs: Spec[Has[ApplicationService] with Blocking with Has[Api] with Clock with Annotations, TestFailure[
    Throwable
  ], TestSuccess]                                                                    =
    suite("Api")(
